-- Re-seeds personal experience demo data for the demo@brewatlas.app account,
-- resolving the user id dynamically from auth.users instead of a hard-coded
-- UUID (the account may have been created after the original seed migration
-- shipped, or with a different id). Skips gracefully when the demo account
-- does not exist, and is idempotent when data is already present.
...
begin
  ...
  if not exists (select 1 from public.user_coffee_setups where user_id = demo_user_id) then
  ...
  if not exists (select 1 from public.user_taste_profiles where user_id = demo_user_id) then
