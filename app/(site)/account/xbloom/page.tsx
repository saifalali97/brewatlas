...
export default async function DashboardXBloomPage() {
  ...
  if (!data.user) {
    redirect("/login?redirectTo=/account/xbloom");
  }
...
<PageHeader headingId="dashboard-xbloom-page-heading" eyebrow={x.eyebrow} title={x.title} description={x.description} centered={false} />
      ...
      {profiles.length === 0 ? (
        <div className="rounded-[1.5rem] border border-ba-espresso/10 bg-ba-pearl px-8 py-16 text-center">
          <p className="text-lg font-medium text-ac-espresso">{x.noProfilesTitle}</p>
          <p className="mt-2 text-sm text-ac-espresso">{x.noProfilesDescription}</p>
