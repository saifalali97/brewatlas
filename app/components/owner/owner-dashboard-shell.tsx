...
import { buildOwnerNavItems, isOwnerNavActive } from "@/lib/owner/nav";
...
export function OwnerDashboardShell({
  ...
  const navItems = buildOwnerNavItems(dictionary.ownerNav);
      ...
      <nav aria-label={labels.sidebarAriaLabel} className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1">
          {navItems.map(({ id, href, label, icon: Icon }) => {
            const active = isOwnerNavActive(pathname, href);
            return (
              <li key={id}>
                <Link
                  href={href}
        ...
        <div className="min-w-0 flex-1">
          <header className="sticky top-0 z-40 border-b border-white/[0.08] bg-[#0a0705]/90 backdrop-blur-2xl">
            <div className="flex items-center justify-between gap-4 px-4 py-3 sm:px-6">
              ...
              <div className="flex items-center gap-3">
                <NotificationsBell
                  key={`owner-${unreadCount}-${notifications.map((item) => item.id).join(",")}`}
                  userId={userId}
          ...
          <main id="owner-dashboard-content" className="px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
            {children}
          </main>
        </div>
      </div>
