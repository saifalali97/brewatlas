...
type GulfHeritagePageCardProps = {
...
/** Category-hub card for an individual Gulf Heritage page. */
export function GulfHeritagePageCard({
  countrySlug,
  categorySlug,
  pageSlug,
  copy,
    ...
    <article className={cards.premiumShell}>
      ...
      <div className="relative flex flex-1 flex-col p-5 lg:p-6">
        ...
        <div className="mt-3 flex items-start gap-2">
          ...
          <p className="text-[0.8125rem] leading-[1.65] text-ac-espresso">{readLabel}</p>
        </div>

        <div className="mt-auto border-t border-ba-espresso/06 pt-4">
          <GhostCtaLink href={gulfHeritagePagePath(countrySlug, categorySlug, pageSlug)}>{readLabel}</GhostCtaLink>
        </div>
      </div>
    </article>
  );
