...
import { AdminUserRowActions } from "@/app/components/admin/users/admin-user-row-actions";
import { adminCopy } from "@/lib/admin/copy";
import { buttons } from "@/lib/constants/styles";
import type { AdminUsersPageResult } from "@/lib/data/admin-users";
...
type AdminUsersExplorerProps = {
  result: AdminUsersPageResult;
  actorId: string;
  filters: {
    search: string;
...
function buildQuery(pathname: string, filters: AdminUsersExplorerProps["filters"], page: number): string {
...
export function AdminUsersExplorer({ result, actorId, filters }: AdminUsersExplorerProps) {
  ...
  const applyFilters = useCallback(
    (next: Partial<AdminUsersExplorerProps["filters"]>) => {
    ...
    <div className="mt-8 space-y-6">
      ...
      >
        ...
        <div>
          ...
          <select id="admin-user-status" name="status" defaultValue={filters.status} className={selectClass}>
          ...
          <table className="min-w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/[0.08] text-xs uppercase tracking-[0.12em] text-stone-500">
                ...
                <th className="px-4 py-3">{labels.columnEmail}</th>
            ...
            <tbody>
              {result.items.map((user) => (
                <tr key={user.id} className="border-b border-white/[0.05] text-stone-300">
                  ...
                  <td className="px-4 py-4 text-xs text-stone-400">{user.email ?? "—"}</td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <AdminUserRowActions user={user} actorId={actorId} labels={labels} />
                    </div>
                  </td>
                </tr>
              ))}
